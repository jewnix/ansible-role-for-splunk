...
- **configure_systemd.yml** - Creates a systemd service override file for Splunk with configurable resource limits. When `systemd.splunk.override` is set to `true`, this task creates a drop-in configuration file at `/etc/systemd/system/<service>.service.d/limits.conf` that overrides systemd unit limits for optimal Splunk performance. Limits can be customized or disabled via the `systemd.splunk.*` variables in group_vars or host_vars. Default: true for backwards compatibility.
