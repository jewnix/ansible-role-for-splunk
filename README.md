...
1. Install Ansible (version >=v2.11 is supported and should work through v2.19)
...
- **configure_splunk_secret.yml** - Configures a common splunk.secret file so that pre-hashed passwords can be securely deployed. The secret can be provided either as a vault-encrypted variable (`splunk_secret_content`) or from a file in the role's files directory (`splunk_secret_file`). When `splunk_secret_content` is non-empty, it takes precedence over the file; otherwise the file is used. Note that changing splunk.secret will require re-encryption of any passwords that were encrypted using the previous splunk.secret since Splunk will no longer be able to decrypt them successfully.
